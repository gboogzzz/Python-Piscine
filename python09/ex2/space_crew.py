...
class SpaceMission(BaseModel):
    ...
    def validate_rules(self) -> SpaceMission:
        ...
        members_exp = [m for m in self.crew if m.years_experience >= 5]
        if self.duration_days > 365 and len(members_exp) < len(self.crew) / 2:
            raise ValueError(
                "Long missions (> 365 days) need 50% "
                "experienced crew (5+ years)"
            )
        active = [member for member in self.crew if member.is_active]
        if len(active) < len(self.crew):
